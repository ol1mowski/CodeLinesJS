...
export const RegexRaiderGame = memo(
  ({ onScoreUpdate, onLevelComplete, currentLevel, onGameOver }: RegexRaiderGameProps) => {
    ...
    const handleSubmit = useCallback(() => {
      try {
        ...
        const correctMatches: string[] =
          currentChallenge.text.match(new RegExp(currentChallenge.correctRegex, 'g')) || [];
            ...
            >
              ...
              <div className="space-y-1">
                {matches.map((match, index) => (
                  <div key={index} className="font-mono text-js">
                    {match}
                  </div>
                ))}
              </div>
