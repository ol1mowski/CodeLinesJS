...
export const ScopeExplorerGame = memo(
  ({
    ...
    return (
      ...
      >
        <ScopeExplorerProgress currentLevel={currentLevel} totalLevels={totalLevels} />
        ...
        <div className="grid grid-cols-3 gap-4">
          {currentChallenge.options.map((option, index) => (
            ...
            `}
            >
              <span className="absolute top-2 left-2 text-xs text-gray-500">{index + 1}</span>
              {option}
            </motion.button>
          ))}
        </div>
        ...
        {showHint && !isCorrect && (
          ...
          >
            <p className="text-sm text-js">
              <span className="font-medium">Podpowiedź:</span> Zwróć uwagę na zakres zmiennych i ich
              dostępność w różnych kontekstach.
            </p>
          </motion.div>
        )}
      </motion.div>
