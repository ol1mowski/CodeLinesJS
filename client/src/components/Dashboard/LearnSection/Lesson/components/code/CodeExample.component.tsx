...
export const CodeExample = memo(({ example, index }: CodeExampleProps) => {
  if (!example.code) {
    return (
      <div className="p-4 bg-dark-800/50 rounded-lg border border-js/10">
        <p className="text-gray-400 text-sm text-center">Ten przykład nie zawiera jeszcze kodu.</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      <h3 className="text-lg font-medium text-gray-200">Przykład {index + 1}:</h3>
      <CodeBlock
        code={example.code}
      ...
      />
      {example.explanation && <p className="text-gray-400 text-sm">{example.explanation}</p>}
    </div>
  );
});
