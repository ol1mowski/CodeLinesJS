        ...
        <div className="space-y-4">
          <div>
            <div className="flex items-start justify-between gap-4 mb-2">
              <div className="flex-1">
                ...
                <p
                  className={`text-sm mt-1 line-clamp-2 ${isLocked ? 'text-gray-500' : 'text-gray-400'}`}
                >
                  {description}
                </p>
              </div>
              <span
                ...
                ${isLocked ? 'bg-gray-700/30 text-gray-400' : 'bg-js/10 text-js'}`}
              >
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3 text-sm">
              <span
                className={`flex items-center gap-1 ${isLocked ? 'text-gray-500' : 'text-gray-400'}`}
              >
            ...
            {isLocked ? (
              <span className="text-xs text-gray-500">Dostępne od poziomu {requiredLevel}</span>
            ) : (
              <Link
                to={`/dashboard/learn/lesson?lessonId=${id}`}
                state={{ lessonId: id }}
                className={`p-2 rounded-lg transition-colors ${
                  isCompleted ? 'text-green-400 hover:bg-green-500/10' : 'text-js hover:bg-js/10'
                }`}
              >
