...
export const RewardNotification = memo(
  ({ reward, isVisible, onClose }: RewardNotificationProps) => {
          ...
          >
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                <h4 className="text-js font-medium mb-1">{reward.title}</h4>
                <p className="text-sm text-gray-400">{reward.description}</p>
                {reward.type === 'xp' && <p className="text-sm text-js mt-1">+{reward.value} XP</p>}
              </div>
