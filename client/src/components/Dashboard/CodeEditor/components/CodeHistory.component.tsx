...
export const CodeHistory = memo(({ history, onSelect, onClear }: CodeHistoryProps) => (
  <div
    ...
    className="absolute top-full right-0 mt-2 w-64 bg-dark/95 rounded-lg border border-js/10 shadow-xl"
  >
    ...
    <div className="max-h-64 overflow-y-auto">
      {history.length === 0 ? (
        <div className="p-3 text-sm text-gray-400">Brak historii</div>
      ) : (
        history.map((entry, index) => (
          ...
          >
            <div className="text-sm text-gray-300 truncate">{entry.code.split('\n')[0]}</div>
