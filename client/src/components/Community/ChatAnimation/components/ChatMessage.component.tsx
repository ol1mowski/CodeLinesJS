...
export const ChatMessage = memo(
  ({ user, avatar, time, displayedMessage, isCurrentMessage, cursorVisible }: ChatMessageProps) => (
    <motion.div
      initial={{ opacity: 0 }}
    ...
    >
      ...
      <div className="flex-1">
        ...
        <div className="p-3 rounded-lg bg-dark/50 border border-js/10 text-gray-300">
          ...
          {isCurrentMessage && (
            <span
              className={`ml-0.5 -mt-1 text-gray-300 ${cursorVisible ? 'opacity-100' : 'opacity-0'}`}
            >
              |
            </span>
          )}
