...
describe('ResetPasswordForm', () => {
  ...
  it('Renders the form correctly with a valid token', () => {
    ...
    const hasNewPasswordLabel = Array.from(newPasswordLabels).some(label =>
      label.textContent?.toLowerCase().includes('nowe hasło')
    );
    ...
    const hasConfirmPasswordLabel = Array.from(confirmPasswordLabels).some(label =>
      label.textContent?.toLowerCase().includes('potwierdź nowe hasło')
    );
  ...
  it('waliduje wymagane pola', async () => {
    ...
    await waitFor(() => {
      ...
      const hasErrorMessage = Array.from(errorElements).some(element =>
        element.textContent?.toLowerCase().includes('hasło musi mieć')
      );
  ...
  it('displays a success message after a successful password reset', async () => {
    ...
    await waitFor(() => {
      ...
      const hasSuccessMessage = Array.from(successElements).some(element =>
        element.textContent?.toLowerCase().includes('hasło zostało pomyślnie zresetowane')
      );
  ...
  it('displays an error message when the password reset fails', async () => {
    ...
    await waitFor(() => {
      ...
      const hasErrorMessage = Array.from(errorElements).some(element =>
        element.textContent?.toLowerCase().includes('token resetowania hasła wygasł')
      );
      expect(hasErrorMessage).toBe(true);
    });
  });
